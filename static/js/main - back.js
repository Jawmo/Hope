...
function output(jobj) {
  ...
  if (jobj.jtype.type == "attr") {
    ...
    // const width = 4; // x max
    // const height = 3; // y max

    // player position
    for (i in jpayload) {

        const user_location = jpayload[i].user_location;
        const position_x = jpayload[i].x;
        const position_y = jpayload[i].y * -1;
        const pos = `${position_x}|${position_y}`;
        data[pos] = Object.assign({}, jpayload[i]);
        if (user_location)
        playerPos = {x: position_x, y: position_y};
    }

    // Line container.
    const lines = [];

    // Loop through our map data and build each line.
    for (let y = playerPos.y-5; y <= 3; y++) {
        let t = " ";
        for (let x = playerPos.x-5; x <= 3; x++) {
            const pos = `${x}|${y}`;
            const tile = data[pos];
            if (tile !== undefined)
                t += tile.symbol.replace(/&/, "&amp;").replace(/</, "&lt;").
                  replace(/>/, "&gt;").replace(/"/, "&quot;").
                  replace(/\|self_text\|/g, '<span class="map_padding" style="color: #7f7cff;">')       .replace(/\|self_textx\|/g, '</span>').
                  replace(/\|map_none\|/g, '<span class="map_padding" style="color: #7f7cff;">')       .replace(/\|map_nonex\|/g, '</span>').
                  replace(/\|y\|/g, '<span class="map_padding" style="color: #ffe100;">')               .replace(/\|yx\|/g, '</span>'); // "
            else
                t += '<span class="map_padding" style="color: black;">o</span>';
        }
        lines.push(t);
    }

    console.log(lines.join("\n"));
    print_map.innerHTML = lines.join("\n");
